...
int hamming_distance(char* buf1, char* buf2, int len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        total += __builtin_popcount(diff_bits);
    }
...
char* black_box(char* str, int len, int* out_len, char* key) {
    /*
    ...
    int append_size = 0;
    char* append_bytes = unbase64(append_str, strlen(append_str), &append_size);
    int total_len = append_size + len;
    char* all_bytes = malloc(total_len);
    memcpy(all_bytes, str, len);
    memcpy(all_bytes + len, append_bytes, append_size);
    free(append_bytes);
